...
#a <- '/home/song7602/3.workbench/RData/ALL_STAR_EXPR_RData_Sample_Sheet.txt'
a <- '/home/song7602/3.workbench/RData/EGA_STAR_EXPR_RData_Sample_Sheet.txt'
b <- read.table(a, fill=TRUE, sep = "\t", col.names = paste0("V", seq(9)))
dim(b)

# 공백 제거
b$V2 <- trimws(b$V2)
# 점을 이스케이프하고 패턴을 다시 적용
...
files <- unique(b[m, "V2"])
names <- unique(b[m, "V1"])
D <- loadStarExpr(files, names, min.umis=1000, min.genes=500, cells=m2)
...
id2chr <- read.table("/home/song7602/1.scripts/aml2019/02_Gene_quantification/hg38.RefSeq_refGene.gtf.transcripts")   # mapping of gene IDs to chromosomes
gene2id <- read.table("/home/song7602/1.scripts/aml2019/02_Gene_quantification/hg38.refMrna.gene.chrM.ercc.fa.names", fill = NA)   # mapping of gene names to gene IDs
...
## 오류 발생하여 수정한것 
# dgeMatrix를 일반 매트릭스로 변환
X_matrix <- as.matrix(X)
# 상관계수 계산 (NA 값을 무시)
X.cor <- cor(X_matrix, use = "pairwise.complete.obs")
# 결과 확인
