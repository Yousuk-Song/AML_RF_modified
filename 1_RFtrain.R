...
setwd("/home/song7602/3.workbench/RData")
...
load("BM_STAR_EXPR_RData_Sample_Sheet.txt.RData", verbose = TRUE)  # as generated using the prepareBackSpinKNN script

X <- log(E+1)  # filtered gene set
dim(X)

# load BackSPIN cell type annotation
a <- read.table("/home/song7602/1.scripts/aml2019/04_Random_forest_classifier/BM_6915cells.BackSPIN.txt", row.names=1, header=TRUE)  # cell annotations available on GEO
head(a)
#                   cluster
...
rf.all.cv <- lapply(cv, function(y2) {
  ...
  # 각 클래스의 샘플 크기 계산
  class_counts <- table(y[setdiff(x, y2)])
  
  # sampsize를 각 클래스의 최소 샘플 수로 설정
  min_class_size <- min(class_counts)
  
  randomForest(x = t(X[rf.all.outer.used1k, setdiff(x, y2)]),
               y = y[setdiff(x, y2)],
               sampsize = rep(min_class_size, length(unique(y))), # 50 대신, 최소 클래스 크기로 설정 --> 왜 50이 에러나는지는 의문..
               ntree = 1000,
               do.trace = TRUE)
})
#rf.all.cv <- lapply(cv, function(y2) {
...
rf.all.cv.predict.prob <- lapply(rf.all.cv, function(rf) {
  predict(rf, t(X[rf.all.outer.used1k, rownames(a)]), type = "prob")
})
...
load("AML939_STAR_EXPR_RData_Sample_Sheet.txt.RData", verbose = TRUE)    # as generated using the prepareBackSpinKNN script
Y <- log(E+1)
...
# reclassify Prog with highest score for HSC to HSC to have 65 cells in total ### 오류 해결을 위해 수정

if (table(rf.all.inner.mut.max)["HSC"] < 65) {
  ...
  rf.all.inner.mut.prog2hsc <- NULL  # 이미 HSC가 충분하므로 재분류 필요 없음
}
#rf.all.inner.mut.prog2hsc <- names(sort(rf.all.inner.mut[rf.all.inner.mut.max == "Prog", "HSC"], decreasing = TRUE)[1:(65-table(rf.all.inner.mut.max)["HSC"])])
...
rf.tumor.cv <- lapply(cv, function(y2) {
  ...
  # 샘플에서 빈 클래스 제거
  y_sub <- y[setdiff(x, y2)]
  
  # 빈 클래스 제거
  y_sub <- droplevels(y_sub)
  
  # 클래스별로 1개 이상 존재하는 클래스만 사용
  valid_classes <- y_sub[table(y_sub)[y_sub] > 0]
  
  # 각 클래스별 샘플 수 계산
  class_counts <- table(valid_classes)
  min_class_size <- min(class_counts)
  
  randomForest(x = t(XY[rf.tumor.outer.used1k, names(valid_classes)]),
               y = valid_classes,
               sampsize = rep(min_class_size, length(unique(valid_classes))),
               ntree = 1000,
               do.trace = TRUE)
})
#rf.tumor.cv <- lapply(cv, function(y2) {
