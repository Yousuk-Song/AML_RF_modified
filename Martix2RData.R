...
convert_files_to_RData <- function(input_dir, output_dir) {
  ...
  for (file in files) {
    ...
    # d.stats 생성 및 추가
    d.stats <- data.frame(matrix(ncol = 4, nrow = ncol(d)))
    colnames(d.stats) <- c("umi", "umi.ribo", "umi.mito", "umi.ercc")
...
input_directory <- "/data/rawdata/scRSEQ_AML/SRA/matrix"
output_directory <- "/data/rawdata/scRSEQ_AML/SRA/Rdata"
